...
lead-pipeline.py: Apollo sourcing → LeadMagic verification → Instantly dedupe；执行前强制 verified + signal-backed audience gate。
competitive-monitor.py: 采集竞品定价与内容差异，并生成招聘信号检索提示。
cross-signal-detector.py: 跨数据源聚合公司、行业与关键词信号。
cold-outbound-sender.py: 验证 audience batch、审批 manifest、recipient identity、限额与内容后执行邮件发送。
...
- 所有 destination 与 sender 必须先通过 `thirtyx.audience`，不得把“有 email”当成 verified。
